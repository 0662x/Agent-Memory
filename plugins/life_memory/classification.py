...
_CHINESE_TECHNICAL_RE = re.compile(r"(接口|依赖|数据库|仓库|分支|提交|调试|服务器|运行时|插件|测试|代码)")

_USER_PROFILE_RE = re.compile(
    r"\b("
    r"from now on|always|whenever|when you|generate|respond|reply|answer|"
    r"write|format|tone|style|language|translation|translate|include .* below|"
    r"call me|refer to me|my long[- ]term goal"
    r")\b",
    re.I,
)
_CHINESE_USER_PROFILE_RE = re.compile(r"(从现在开始|以后你|回答时|回复时|翻译|叫我|称呼我|输出格式)")
...
_LIFE_MEMORY_RE = re.compile(
    r"\b("
    r"remember|prefer|preference|favorite|favourite|like|love|enjoy|"
    ...
    r"live|lives|home|birthday|pet|dog|cat|exercise|sleep|work late|"
...
_CHINESE_LIFE_MEMORY_RE = re.compile(
    ...
    r"我住在|我搬到|我的(姐姐|妹妹|哥哥|弟弟|妈妈|爸爸|伴侣|妻子|丈夫|家人|朋友)|生日|宠物|睡眠|运动)"
)
...
_PREFERENCE_RE = re.compile(
    r"\b(prefer|preference|favorite|favourite|rather|like|love|enjoy)\b",
    re.I,
)
...
_CHINESE_PATTERN_RE = re.compile(r"(通常|一般|经常|往往|习惯|总是)")
...
_CHINESE_FOOD_RE = re.compile(r"(咖啡|茶|餐厅|早餐|午餐|晚餐)")
_CHINESE_HEALTH_RE = re.compile(r"(运动|睡眠|累|健康|药|治疗|焦虑)")
